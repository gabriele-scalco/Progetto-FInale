...
class PriceDescendingStrategyTest {

    /**
     * Verifica che le biciclette siano ordinate correttamente in base al prezzo in ordine decrescente.
     */
    @Test
    void testSort() {
        ...
        PriceDescendingStrategy strategy = new PriceDescendingStrategy();

        List<Bike> sortedBikes = strategy.sort(bikes);

        assertEquals(bike3, sortedBikes.get(0)); // Verifica che la bici più costosa sia la prima
        assertEquals(bike1, sortedBikes.get(1)); // Verifica che la bici intermedia sia la seconda
        assertEquals(bike2, sortedBikes.get(2)); // Verifica che la bici più economica sia l'ultima
    }
